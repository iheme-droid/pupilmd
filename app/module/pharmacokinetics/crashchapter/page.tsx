...
function PharmacokineticsContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const currentSectionId = parseInt(searchParams.get('section') || '0');
  const currentSection = pharmacokineticsChapter.sections.find(s => s.id === currentSectionId);
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`pupilmd_teaching_receipt_${pharmacokineticsChapter.moduleSlug}`);
    if (saved) setCheckedQuestions(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem(`pupilmd_progress_${pharmacokineticsChapter.moduleSlug}`, JSON.stringify({
  ...
  const handleQuestionToggle = (index: number) => {
    ...
    localStorage.setItem(`pupilmd_teaching_receipt_${pharmacokineticsChapter.moduleSlug}`, JSON.stringify(newChecked));
  };

  const goToSection = (id: number) => {
    router.push(`/module/pharmacokinetics/crashchapter?section=${id}`);
  ...
  if (!currentSection) return (
    <div style={{ padding: '48px 24px' }}>
      ...
      <Link href="/module/pharmacokinetics" style={{ color: '#5856D6' }}>Back to module</Link>
    </div>
  );
    ...
    <div style={{ background: isDeepDive ? '#FFF8EE' : '#F7F8FA', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
          {pharmacokineticsChapter.sections.map(section => {
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 30, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
        <div style={{ height: '3px', background: '#ECEEF2' }}>
          <div style={{ height: '100%', width: `${progressPct}%`, background: isDeepDive ? 'linear-gradient(90deg, #FF9500, #FF6B35)' : `linear-gradient(90deg, ${accent}, #0A84FF)`, transition: 'width 0.5s ease' }} />
        </div>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '36px', height: '36px', borderRadius: '10px', flexShrink: 0, background: isDeepDive ? 'rgba(255,149,0,0.1)' : 'rgba(88,86,214,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Sora', sans-serif", fontWeight: 800, fontSize: '0.85rem', color: isDeepDive ? '#FF9500' : accent }}>
              {currentSectionId}
            </div>
        ...
        {showTeachingReceipt && (
          <TeachingReceipt
            mastered={pharmacokineticsChapter.teachingReceipt.mastered}
            selfCheckQuestions={pharmacokineticsChapter.teachingReceipt.selfCheckQuestions}
    ...
    Back to Top
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', paddingTop: '24px', borderTop: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' as const, justifyContent: 'center', flex: 1 }}>
            {pharmacokineticsChapter.sections.map(s => (
...
export default function PharmacokineticsCrashChapter() {
  return (
    <Suspense fallback={
