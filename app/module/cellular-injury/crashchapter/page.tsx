...
export default function CrashChapterReader() {
  ...
  const moduleSlug = 'cellular-injury';

  const currentSection = cellularInjuryChapter.sections.find(
    s => s.id === currentSectionId
  );

  const nextSection = cellularInjuryChapter.sections.find(
    s => s.id === currentSectionId + 1
  );

  const prevSection = cellularInjuryChapter.sections.find(
    s => s.id === currentSectionId - 1
  );
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <ProgressBar 
          currentSection={currentSectionId} 
          totalSections={cellularInjuryChapter.sections.length} 
        />
      </div>
